...
@section('content')
    <div class="page-header"
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
            <a href="{{ route('admin.lansia.export') }}" class="btn btn-success"
    ...
    <div class="stats-grid" style="margin-bottom:24px;">
        ...
        <div class="stat-card success">
            ...
            <div style="font-size:12px;color:var(--text-tertiary);margin-top:6px;display:flex;gap:10px;">
                <span><i class="fas fa-mars" style="color:var(--brand-400);"></i>
                    {{ number_format($grandPendudukL) }}</span>
        ...
        <div class="stat-card danger">
            ...
            <div class="stat-card__value">
                {{ number_format($lansias->sum('usia_70_plus_l') + $lansias->sum('usia_70_plus_p')) }}</div>
            <div style="font-size:12px;color:var(--text-tertiary);margin-top:6px;">
                <span><i class="fas fa-mars" style="color:var(--brand-400);"></i>
                    {{ number_format($lansias->sum('usia_70_plus_l')) }}</span>
                <span style="margin-left:8px;"><i class="fas fa-venus" style="color:#be185d;"></i>
                    {{ number_format($lansias->sum('usia_70_plus_p')) }}</span>
            </div>
        </div>
    </div>
        ...
        <div class="card__header" style="flex-wrap:wrap;gap:12px;">
            <h3 class="card__title">
                ...
                <span class="badge badge--brand" style="margin-left:8px;font-size:11px;"
                    id="countBadge">{{ $lansias->count() }}</span>
            </h3>
                                ...
                                <td>
                                    <div style="font-weight:700;color:var(--text-primary);font-size:14px;">
                                        <i class="fas fa-location-dot"
                                            style="color:var(--brand-400);font-size:11px;margin-right:5px;"></i>
                                        {{ $row->desa }}
                                    </div>
                                </td>
                                <td style="text-align:center;color:var(--brand-700);font-weight:600;">
                                    {{ number_format($row->jumlah_penduduk_l) }}</td>
                                <td style="text-align:center;color:#be185d;font-weight:600;">
                                    {{ number_format($row->jumlah_penduduk_p) }}</td>
                                <td
                                    style="text-align:center;font-weight:800;font-size:15px;color:var(--text-primary);background-color:var(--gray-50);">
                                    {{ number_format($row->jumlah_penduduk_l + $row->jumlah_penduduk_p) }}</td>
                                <td style="text-align:center;color:var(--brand-600);">
                                    {{ number_format($row->usia_60_69_tahun_l) }}</td>
                                <td style="text-align:center;color:#be185d;">{{ number_format($row->usia_60_69_tahun_p) }}
                                </td>
                                <td style="text-align:center;color:var(--brand-600);">
                                    {{ number_format($row->usia_70_plus_l) }}</td>
                                <td style="text-align:center;color:#be185d;">{{ number_format($row->usia_70_plus_p) }}
                                </td>
                                <td style="text-align:center;color:var(--brand-600);font-weight:700;">
                                    {{ number_format($row->usia_60_69_tahun_l + $row->usia_70_plus_l) }}</td>
                                <td style="text-align:center;color:#be185d;font-weight:700;">
                                    {{ number_format($row->usia_60_69_tahun_p + $row->usia_70_plus_p) }}</td>
                                <td>
                                    <div style="display:flex;gap:5px;justify-content:center;">
                                        <a href="{{ route('admin.lansia.edit', $row) }}" class="btn btn-outline btn-sm"
                                            title="Edit">
                    ...
                    <tfoot>
                        <tr style="background:var(--brand-950);color:#fff;font-weight:800;">
                            ...
                            <td style="text-align:center;font-size:15px;">
                                {{ number_format($totalPendudukL + $totalPendudukP) }}</td>

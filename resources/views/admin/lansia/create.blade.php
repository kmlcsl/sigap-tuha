...
@section('content')
    <style>
        ...
        .pl-usia-table tbody tr:last-child {
            border-bottom: none;
        }

        .pl-usia-table tbody tr:hover {
            background: rgba(29, 78, 216, .05);
        }

        .pl-usia-table tbody tr:nth-child(even) {
            background: rgba(241, 245, 249, .7);
        }

        .pl-usia-table tbody tr:nth-child(even):hover {
            background: rgba(29, 78, 216, .05);
        }
        ...
        .pl-usia-table td.input-cell input.is-l:focus {
            border-color: #3b6cf9;
            box-shadow: 0 0 0 3px rgba(59, 108, 249, .12);
        }

        .pl-usia-table td.input-cell input.is-p:focus {
            border-color: #be185d;
    ...
    <div class="pl-form-wrap">
        ...
        <div class="card" style="padding:0;overflow:hidden;margin-bottom:20px;">
            <div style="background:linear-gradient(135deg,#0b2c6b,#1d4ed8);padding:22px 28px;color:#fff;">
                ...
                <p style="margin:0;font-size:13px;opacity:.85;">Isi rekap jumlah penduduk per kelompok usia untuk desa baru.
                </p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.lansia.store') }}" method="POST">
                ...
                <div class="pl-section__body">
                    <div class="form-group" style="max-width:420px;margin-bottom:0;">
                        ...
                        <input type="text" id="desa" name="desa"
                            class="form-control @error('desa') is-error @enderror" value="{{ old('desa') }}"
                    ...
                    <div class="pl-penduduk">
                        <div class="form-group" style="margin-bottom:0;">
                            ...
                            <input type="number" name="jumlah_penduduk_l" id="jumlah_penduduk_l"
                                ...
                                value="{{ old('jumlah_penduduk_l', 0) }}" min="0" required readonly
                                style="background-color: #f3f4f6; cursor: not-allowed; opacity: 0.8;">
                            @error('jumlah_penduduk_l')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group" style="margin-bottom:0;">
                            ...
                            <input type="number" name="jumlah_penduduk_p" id="jumlah_penduduk_p"
                                ...
                                value="{{ old('jumlah_penduduk_p', 0) }}" min="0" required readonly
                                style="background-color: #f3f4f6; cursor: not-allowed; opacity: 0.8;">
                            @error('jumlah_penduduk_p')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
            ...
            @php
                $kelompok = [
                    ...
                    [
                        'key' => 'usia_0_11_bulan',
                        'label' => 'Usia 0–11 Bulan',
                        'icon' => 'fa-baby-carriage',
                        'badge_bg' => '#dbeafe',
                        'badge_color' => '#1d4ed8',
                    ],
                    [
                        'key' => 'usia_12_59_bulan',
                        'label' => 'Usia 12–59 Bulan',
                        'icon' => 'fa-child',
                        'badge_bg' => '#cffafe',
                        'badge_color' => '#0e7490',
                    ],
                    [
                        'key' => 'usia_60_72_bulan',
                        'label' => 'Usia 60–72 Bulan',
                    ...
                    [
                        'key' => 'usia_13_14_tahun',
                        'label' => 'Usia 13–14 Tahun',
                        'icon' => 'fa-user',
                        'badge_bg' => '#ffedd5',
                        'badge_color' => '#9a3412',
                    ],
                    [
                        'key' => 'usia_15_59_tahun',
                        'label' => 'Usia 15–59 Tahun',
                        'icon' => 'fa-user-tie',
                        'badge_bg' => '#fee2e2',
                        'badge_color' => '#b91c1c',
                    ],
                    [
                        'key' => 'usia_60_69_tahun',
                        'label' => 'Usia 60–69 Tahun',
                        'icon' => 'fa-user-clock',
                        'badge_bg' => '#fce7f3',
                        'badge_color' => '#9d174d',
                    ],
                    ...
                    <div style="overflow-x:auto;">
                        <table class="pl-usia-table">
                            <thead>
                                <tr>
                                    <th>Kelompok Usia</th>
                                    <th class="center"><i class="fas fa-mars" style="margin-right:5px;"></i>Laki-laki (L)
                                    </th>
                                    <th class="center"><i class="fas fa-venus" style="margin-right:5px;"></i>Perempuan (P)
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($kelompok as $k)
                                    <tr>
                                        <td class="label">
                                            <span>
                                                <i class="fas {{ $k['icon'] }}"
                                                    style="color:{{ $k['badge_color'] }};width:16px;text-align:center;"></i>
                                                <span class="kelompok-badge"
                                                    style="background:{{ $k['badge_bg'] }};color:{{ $k['badge_color'] }};">
                                                    {!! $k['label'] !!}
                                                </span>
                                            </span>
                                        </td>
                                        <td class="input-cell">
                                            ...
                                            @error($k['key'] . '_l')
                                                <div class="form-error" style="font-size:11px;margin-top:3px;">
                                                    {{ $message }}</div>
                                            @enderror
                                        </td>
                                        <td class="input-cell">
            ...
            <div class="pl-section slate">
                <div class="pl-section__head" style="justify-content: space-between;">
                    ...
                    <button type="button" class="btn btn-sm" onclick="addPrioritas()"
                        style="background:#3b6cf9;color:#fff;border:none;padding:4px 10px;border-radius:4px;font-size:11px;font-weight:bold;cursor:pointer;">

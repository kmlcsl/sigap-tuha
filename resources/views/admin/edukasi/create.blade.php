...
@section('content')
    ...
    <div class="page-header">
        <h1 class="page-header__title">Tambah Materi Edukasi</h1>
        <p class="page-header__desc">Tambahkan materi edukasi baru ke sistem.</p>
    </div>
    <div class="card" style="max-width:900px;">
        <div class="card__header">
            <h3 class="card__title"><i class="fas fa-plus-circle"></i> Form Materi Baru</h3>
        </div>
        <form action="{{ route('admin.edukasi.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="judul" id="judul" class="form-control" value="{{ old('judul') }}"
                    required placeholder="Judul materi edukasi">
                @error('judul')
            ...
            <div class="form-group">
                ...
                <input type="text" name="slug" id="slug" class="form-control" value="{{ old('slug') }}"
                    placeholder="contoh: dasar-bantuan-hidup">
                ...
                <div class="form-group">
                    ...
                    <select name="kategori" class="form-control" required>
                        @foreach (['BHD', 'Evakuasi', 'Pertolongan Pertama', 'Lainnya'] as $k)
                            <option value="{{ $k }}" {{ old('kategori', 'BHD') == $k ? 'selected' : '' }}>
                                {{ $k }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="jenis" class="form-control" required>
                        @foreach (['Artikel', 'Video', 'SOP'] as $j)
                            <option value="{{ $j }}" {{ old('jenis', 'Artikel') == $j ? 'selected' : '' }}>
                                {{ $j }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="form-group">
                ...
                <input type="url" name="url_video" id="url_video" class="form-control" value="{{ old('url_video') }}"
            ...
            <div class="form-group">
                <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                    ...
                    <input type="checkbox" name="is_published" value="1"
                        ...
                        style="width:18px; height:18px; accent-color:var(--brand-600);">
                    <span><i class="fas fa-eye" style="margin-right:4px; color:var(--text-tertiary)"></i> Publikasikan
                        langsung</span>
                </label>
            </div>
            <div
